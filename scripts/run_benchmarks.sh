...
# so optimization iterations can be compared: benchmarks/results/<branch>_<sha>.json
#
...
WORKTREE="$worktree" SIZES="$SIZES" REPEATS="$REPEATS" python3 - <<'EOF'
...
doc = {
    ...
    "julia_boundary": json.load(open(os.environ["JULIA_JSON"])),
}
